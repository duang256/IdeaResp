...
import com.easylife.util.MD5Util;
...
public class UserServiceImpl implements IUserService {
    ...
    public ServerResponse<User> loginbytel(String userphone) {
        int resultCount = userMapper.checkUserphone(userphone);
        if (resultCount == 0) {
            return ServerResponse.createByErrorMessage("用户名不存在");
        }
        User user = userMapper.selectLoginbytel(userphone);
        return ServerResponse.createBySuccess("登录成功", user);
    }
    ...
    public ServerResponse<User> addUserYidou(Integer userId) {
        User user = userMapper.selectByPrimaryKey(userId);
        user.setYidou(user.getYidou() + 5);
    ...
    public User getUserInfo(int userId) {
        User user = userMapper.selectByPrimaryKey(userId);
        this.addUserYidou(userId);
        return user;
    }

    private void addUserYidou(int userId) {
        User user = userMapper.selectByPrimaryKey(userId);
        user.setYidou(user.getYidou() + 5);
        userMapper.updateByPrimaryKeySelective(user);
    }
