...
import com.easylife.common.enums.OrderStatusEnum;
import com.easylife.common.enums.PayPlatformEnum;
import com.easylife.common.enums.PaymentTypeEnum;
import com.easylife.common.enums.ProductStatusEnum;
import com.easylife.dao.*;
import com.easylife.pojo.*;
import com.easylife.service.IOrderService;
import com.easylife.util.BigDecimalUtil;
import com.easylife.util.DateTimeUtil;
import com.easylife.util.PropertiesUtil;
import com.easylife.vo.OrderItemVo;
import com.easylife.vo.OrderProductVo;
import com.easylife.vo.OrderVo;
import com.easylife.vo.ShippingVo;
...
public class OrderServiceImpl implements IOrderService {
    ...
    public ServerResponse createOrderByCard(Integer userId, Integer shippingId,int paymentType,String delivery){
        ...
        //生成订单
        Order order = this.assembleOrder(userId, shippingId, payment, paymentType,delivery);
        if(order == null){
        ...
        //生成成功,扣除易豆
        // hua 2021.1.18
        this.reduceUserYidou(orderItemList);
    ...
    public ServerResponse createOrderDirect(int userId, long productId, int quantity, Integer shippingId, int paymentType, String delivery) {
        ...
        BigDecimal payment = BigDecimalUtil.mul(product.getYi_price().doubleValue(), quantity);   //hua 2020.12.29

        Integer Totalyidou=product.getYidou().intValue()*quantity;   //hua 2020.12.29
        //Order订单插入,直接购买
//        ========================================================================
        Order order = this.assembleOrder(userId, shippingId, payment, paymentType,delivery);
        if (order == null) {
        ...
        orderItem.setCurrentUnitPrice(product.getYi_price());   //hua 2020.12.29
        orderItem.setProductyidou(product.getYidou());      //hua 2020.12.29
        orderItem.setQuantity(quantity);
        orderItem.setTotalPrice(payment);
        orderItem.setTotalyidou(Totalyidou);   //hua 2020.12.29
        orderItem.setOrderNo(order.getOrderNo());
    ...
    private OrderItemVo assembleOrderItemVo(OrderItem orderItem){
        ...
        orderItemVo.setProductYidou(orderItem.getProductyidou());  //hua 2020.12.29
        orderItemVo.setQuantity(orderItem.getQuantity());
        orderItemVo.setTotalPrice(orderItem.getTotalPrice());
        orderItemVo.setTotalyidou(orderItem.getTotalyidou());//hua 2020.12.29
...
//hua 2021.1.18
    private void reduceUserYidou(List<OrderItem> orderItemList){
        for(OrderItem orderItem : orderItemList){
            User user = userMapper.selectByPrimaryKey(orderItem.getUserId());
            user.setYidou(user.getYidou()-orderItem.getTotalyidou());
            userMapper.updateByPrimaryKeySelective(user);
        }
    }

    private Order assembleOrder(Integer userId,Integer shippingId,BigDecimal payment,int paymentType,String delivery){
        ...
        //修改支付方式
        if (paymentType == 0) {
            order.setPaymentType(PaymentTypeEnum.WECHAT_PAY.getCode());
        } else {
    ...
    private ServerResponse getCartOrderItem(Integer userId,List<Cart> cartList){
        ...
        for(Cart cartItem : cartList){
            ...
            orderItem.setTotalPrice(BigDecimalUtil.mul(product.getYi_price().doubleValue(),cartItem.getQuantity()));   //hua 2020.12.29
            orderItem.setTotalyidou(product.getYidou()*cartItem.getQuantity());//hua 2020.12.29
            orderItemList.add(orderItem);
        }
    ...
    public ServerResponse<String> pay_Already(Integer userId,Long orderNo){
        Order order  = orderMapper.selectByUserIdAndOrderNo(userId,orderNo);
        if(order == null){
            return ServerResponse.createByErrorMessage("该用户此订单不存在");
        }
        if(order.getStatus() != OrderStatusEnum.NO_PAY.getCode()){
            return ServerResponse.createByErrorMessage("已付款,无需再支付");
        }
        Order updateOrder = new Order();
        updateOrder.setId(order.getId());
    ...
    public ServerResponse getOrderCartProduct(Integer userId){
        ...
        for(OrderItem orderItem : orderItemList) {
            cartTotalyidou=cartTotalyidou+orderItem.getTotalyidou();
        }

        orderProductVo.setProductTotalPrice(payment);
        orderProductVo.setProductTotalYidou(cartTotalyidou);
