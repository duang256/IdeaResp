...
import com.easylife.service.IUserService;
...
public class UserController {
    ...
    @RequestMapping(value = "add_user_Yidou.do")
    @ResponseBody
    public ServerResponse<User> addUserYidou(HttpServletRequest httpServletRequest){
        String loginToken = TokenUtil.readLoginTokenByHeader(httpServletRequest);
        String userJsonStr = RedisShardedPoolUtil.get(loginToken);
        User user = JsonUtil.string2Obj(userJsonStr,User.class);
        iUserService.addUserYidou(user.getId());
       // return iUserService.addUserYidou(user.getId());
     ...
     * @return
     */
    @RequestMapping(value = "loginbytel.do")
    @ResponseBody
    public ServerResponse<User> loginbytel(String userphone){
        ServerResponse<User> response = iUserService.loginbytel(userphone);
        if(response.isSuccess()){
            //1,写token到redis
    ...
    @ResponseBody
    public ServerResponse<UserVo> get_information(HttpServletRequest httpServletRequest){
        String loginToken = TokenUtil.readLoginTokenByHeader(httpServletRequest);
        User user = JsonUtil.string2Obj(RedisShardedPoolUtil.get(loginToken),User.class);
